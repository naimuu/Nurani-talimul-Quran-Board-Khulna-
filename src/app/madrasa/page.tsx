...
import { LayoutDashboard, BookOpen, Users, FileText, LogOut, UserCircle, ChevronDown } from "lucide-react";
      ...
      <aside className="w-64 bg-white border-r border-slate-200 min-h-screen p-4 flex flex-col">
        ...
        <nav className="flex-1 space-y-2">
          {[
            ...
            { icon: Users, label: "শিক্ষার্থী ও শিক্ষক" },
            { icon: BookOpen, label: "একাডেমিক তথ্য" },
            { icon: FileText, label: "নোটিশ ও ফর্ম" },
          ].map((item, idx) => (
            <button
              key={idx}
      ...
      <main className="flex-1 p-8">
        <header className="mb-8 flex justify-between items-center relative z-40">
          <div>
            ...
            <p className="text-slate-500">আপনার মাদরাসার সার্বিক কার্যক্রম এখান থেকে পরিচালনা করুন।</p>
          </div>
        </header>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[
            { label: "মোট শিক্ষার্থী", value: "৪৫০" },
            { label: "শিক্ষক সংখ্যা", value: "১৫" },
            { label: "অপেক্ষমান ফর্ম", value: "৫" },
          ].map((stat, idx) => (
            <motion.div
              key={idx}
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 h-96 flex items-center justify-center">
          <p className="text-slate-400">মূল কন্টেন্ট এরিয়া (মাদরাসার বিস্তারিত)</p>
        </div>
      </main>
    </div>
  );
