...
export async function POST(req: Request) {
  try {
    ...
    const newBatch = await TrainingBatch.create({
      ...
      badge: badge ? badge.trim() : (medium === 'arabic' ? 'আরবি' : 'বাংলা'),
      ...
      muallimId: muallimId || null,
      muallimName: (muallimName || '').trim(),
      muallimDesignation: (muallimDesignation || '').trim(),
      muallimPhone: (muallimPhone || '').trim(),
      muallimPhoto: (muallimPhoto || '').trim(),
      muallimTiming: (muallimTiming || '').trim(),
